<template v-if="!loading">
  <form-schema
    v-if="model && !loading"
    @input="saveEntry(); $emit('input', model)"
    :schema="schema"
    v-model="model"
    @submit="submit"
  >
...
export default {
  ...
  methods: {
    ...mapMutations("JSONschema", ["setSchema", "setEntry"]),
    ...mapActions("n3", ["objectToStore"]),
    saveEntry() {
    ...
    setComponents() {
      ...
      for (let i = 0; i < fields.length; i += 1) {
        if (
          this.schema.properties[fields[i]].attrs &&
